"""
MediArch System Supervisor Graph - 真正并行检索架构版本

这是 MediArch 系统的主 Supervisor 图，负责协调所有 worker agents。
...
logger = logging.getLogger("mediarch_supervisor")
...
DEFAULT_TOP_K = int(os.getenv("ORCH_TOPK_DEFAULT", "8"))
...
def node_init_context(state: SupervisorState) -> Dict[str, Any]:
    """初始化上下文（从 Worker 导入后设置）"""
    # 这个函数会在 build_supervisor_graph 中动态注入 available_workers
    return {}
...
async def node_general_answer(state: SupervisorState) -> Dict[str, Any]:
    """通用回答节点（非医院相关问题）"""
    query = state.get("query", "")
    
    logger.info(f"[Supervisor→GeneralAnswer] 非医院相关问题: {query}")
...
def node_gather_responses(state: SupervisorState) -> Dict[str, Any]:
    """
    ...
    logger.info(f"[Supervisor→Gather] 收集到 {len(items)} 条去重结果")
    logger.info(f"[Supervisor→Gather] 来自 {len(worker_responses)} 个 Worker 的完整响应")
    ...
    logger.info(f"[Supervisor→Gather] Worker 统计: {worker_stats}")
...
def node_save_memory(state: SupervisorState) -> Dict[str, Any]:
    """保存长期记忆和对话历史"""
    ...
    logger.info(f"[Supervisor→SaveMemory] 已保存用户 {user_id} 的记忆")
...
def route_after_mark(state: SupervisorState) -> str:
    ...
    logger.info(f"[Supervisor→Barrier] 完成 {completed}/{expected}")
...
def _get_worker_workflows() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("[Supervisor] 导入 neo4j_agent.graph")
    except Exception as e:
        logger.warning(f"[Supervisor] 无法导入 neo4j_agent.graph: {e}")
    
    try:
        from backend.app.agents.milvus_agent.agent import graph as milvus_graph
        workflows["milvus_agent"] = milvus_graph
        logger.info("[Supervisor] 导入 milvus_agent.graph")
    except Exception as e:
        logger.warning(f"[Supervisor] 无法导入 milvus_agent.graph: {e}")
    
    try:
        from backend.app.agents.mongodb_agent.agent import graph as mongodb_graph
        workflows["mongodb_agent"] = mongodb_graph
        logger.info("[Supervisor] 导入 mongodb_agent.graph")
    except Exception as e:
        logger.warning(f"[Supervisor] 无法导入 mongodb_agent.graph: {e}")
    ...
    try:
        ...
        logger.info("[Supervisor] 导入 online_search_agent.graph")
    except Exception as e:
        logger.warning(f"[Supervisor] 无法导入 online_search_agent.graph: {e}")
...
def create_async_checkpointer():
    ...
    if is_langgraph_api:
        logger.info("[Supervisor->Checkpointer] LangGraph API环境，使用平台内置持久化")
        return None  # LangGraph API会自动处理

    # 使用 MemorySaver（内存持久化，进程重启后丢失）
    # 这是最稳定的选择，避免 AsyncSqliteSaver 的异步上下文管理器问题
    logger.info("[Supervisor->Checkpointer] 使用MemorySaver（内存持久化）")
    logger.warning(
        "[Supervisor→Checkpointer] ⚠️ MemorySaver会产生阻塞调用警告，"
...
def build_supervisor_graph():
    ...
    # ★修改2：新增聊天入口节点
    def node_chat_entry(state: SupervisorState) -> Dict[str, Any]:
        ...
        if request and hasattr(request, 'query') and request.query:
            query = request.query
            logger.info(f"[Supervisor->ChatEntry] 从 request 获取 query: {query[:50]}...")
        else:
            ...
            for m in reversed(msgs):
                if isinstance(m, HumanMessage):
                    query = m.content
                    logger.info(f"[Supervisor->ChatEntry] 从 messages 获取 query: {query[:50]}...")
                    break
    ...
    for worker_name, worker_graph in worker_graphs.items():
        try:
            ...
            logger.info(f"[Supervisor] 使用标准 Adapter 添加 Worker: {worker_name}")
        except Exception as e:
            logger.error(f"[Supervisor] 添加 {worker_name} 失败: {e}")
    
    logger.info(f"[Supervisor] 共添加 {len(workers_added)} 个 Worker: {workers_added}")
    
    # ========== 添加核心节点 ==========
    
    # 1. 上下文初始化
    def node_init_context_impl(_: SupervisorState) -> Dict[str, Any]:
    ...
    # 4. 并行 Worker 调度节点
    def node_prepare_parallel_workers(state: SupervisorState) -> Dict[str, Any]:
        ...
        if not state.get("is_hospital_related", True):
            logger.info("[Supervisor→PrepareParallel] 问题不属于医院建筑领域，跳过 Worker 调用")
            return {
                "scheduled_workers": [],
                "active_workers": [],
                "neo4j_expansion": {},
        ...
        try:
            ...
            if strict_cross_doc_request:
                ...
                logger.info("[Supervisor→PrepareParallel] strict_doc_scope=on, scheduled_workers=%s", workers)
        except Exception as exc:
            logger.warning("[Supervisor→PrepareParallel] strict_doc_scope 检测失败: %s", exc)

        if not workers:
            logger.warning("[Supervisor→PrepareParallel] 没有可用 Worker")
            return {
                "scheduled_workers": [],
                "active_workers": [],
                "neo4j_expansion": {},
        ...
        try:
            ...
            logger.info("[Supervisor→PrepareParallel] QueryExpansion 生成 %s 个搜索词", len(neo4j_expansion["search_terms"]))
        except Exception as exc:
            logger.warning(f"[Supervisor→PrepareParallel] QueryExpansion 失败: {exc}，使用基础扩展")
            neo4j_expansion = {
    ...
    def node_fan_out_workers(_: SupervisorState) -> Dict[str, Any]:
        """占位节点，用于触发并行 Worker。"""
        return {}

    def node_extract_neo4j_expansion(state: SupervisorState) -> Dict[str, Any]:
        """
        ...
        for resp in worker_responses:
            ...
            logger.info(
                "[Supervisor→ExtractExpansion] 采纳 Neo4j 扩展: %s 个实体 / %s 条关系",
...
# 这些节点必须在边定义之前添加
    ...
    def node_knowledge_fusion(state: SupervisorState) -> Dict[str, Any]:
        """
        ...
        logger.info(f"[Supervisor→KnowledgeFusion] 开始融合，共 {len(worker_responses)} 个 Worker 响应")
        ...
        if cached_fusion is not None:
            logger.info("[Supervisor→KnowledgeFusion] 命中缓存，直接使用融合结果")
            return {
        ...
        for resp in worker_responses:
            ...
            if agent_name == "neo4j_agent":
                ...
                logger.info(f"[Supervisor→KnowledgeFusion] Neo4j 贡献: {len(items)} items")
            elif agent_name == "milvus_agent":
                milvus_items.extend(items)
                logger.info(f"[Supervisor→KnowledgeFusion] Milvus 贡献: {len(items)} items")
        ...
        try:
            ...
            logger.info(
                f"[Supervisor→KnowledgeFusion] 融合完成: "
            ...
            logger.error(f"[Supervisor→KnowledgeFusion] 融合失败: {e}")
            # 降级：直接合并 items
    ...
    def node_phase1_barrier(state: SupervisorState) -> Dict[str, Any]:
        """
    ...
    def node_schedule_mongodb(state: SupervisorState) -> Dict[str, Any]:
        """
        ...
        if "mongodb_agent" not in available:
            logger.warning("[Supervisor→ScheduleMongoDB] mongodb_agent 不可用")
            return {
                "scheduled_workers": [],
                "active_workers": [],
            }
        ...
        if strict_cross_doc_request and strict_has_scope:
            ...
            logger.info(
                "[Supervisor→ScheduleMongoDB] strict_cross_doc_request=on，强制调用 MongoDB（doc_scope=%s）",
                len(strict_doc_ids) if strict_doc_ids else len(strict_source_documents),
            )
        elif not chunk_ids and not entity_names:
            logger.info("[Supervisor→ScheduleMongoDB] 无需调用 MongoDB（无 chunk_ids 或 entity_names）")
            return {
                "scheduled_workers": [],
                "active_workers": [],
            }
        ...
        logger.info(
            f"[Supervisor→ScheduleMongoDB] 调度 MongoDB Agent: "
    ...
    try:
        ...
        logger.info("[Supervisor] 添加了 result_synthesizer_agent 子图节点")
    except Exception as e:
        logger.warning(f"[Supervisor] 无法导入 result_synthesizer_agent: {e}")

    # 7. 记忆保存节点
    builder.add_node("save_memory", node_save_memory)

    # 8. 屏障和空节点
    def node_barrier_check(state: SupervisorState) -> Dict[str, Any]:
        logger.info("[Supervisor→Barrier] 触发屏障检查，等待全部 worker 完成")
        return {}
    
    def node_noop(_: SupervisorState) -> Dict[str, Any]:
        return {}
    ...
    def route_after_prepare(state: SupervisorState) -> str:
    ...
    # ========== 阶段1：Neo4j + Milvus 真正并行 ==========
    if workers_added:
        # 只路由阶段1的 Workers（neo4j + milvus）
        phase1_workers_available = [w for w in PHASE1_WORKERS if w in workers_added]

        def route_phase1_workers(state: SupervisorState) -> List[str]:
            """只路由 Neo4j 和 Milvus 进行并行检索"""
            scheduled = state.get("scheduled_workers", []) or []
            # 只选择阶段1的 workers
            phase1_to_run = [w for w in scheduled if w in PHASE1_WORKERS]
            logger.info(f"[Supervisor->Phase1] 并行启动: {phase1_to_run}")
            return phase1_to_run if phase1_to_run else ["noop"]

        # 构建阶段1的路由映射
        phase1_route_mapping: Dict[str, str] = {
            worker: worker for worker in phase1_workers_available
        }
        phase1_route_mapping["noop"] = "noop"

        builder.add_conditional_edges(
            "fan_out_workers",
            route_phase1_workers,
            phase1_route_mapping,
        )

        # 阶段1的 Workers 完成后都进入 phase1_barrier
        for worker in phase1_workers_available:
            builder.add_edge(worker, "phase1_barrier")

    # ========== 阶段1屏障 → Knowledge Fusion ==========
    def route_after_phase1_barrier(state: SupervisorState) -> str:
        """检查阶段1是否完成，决定是否进入融合"""
    ...
    # ========== 阶段2：MongoDB 精确定位 ==========
    def route_after_schedule_mongodb(state: SupervisorState) -> str:
    ...
    # ========== Gather → Synthesizer ==========
    def node_push_answer_message(state: SupervisorState) -> Dict[str, Any]:
    ...
    if not workers_added:
        logger.warning("[Supervisor] 未找到可用 Worker")
    ...
    if checkpointer is None:
        ...
        logger.info("[Supervisor] 图编译完成（LangGraph API环境，使用平台内置持久化）")
    else:
        # 本地环境，使用自定义checkpointer
        compiled_graph = builder.compile(checkpointer=checkpointer)
        logger.info(
            f"[Supervisor] 图编译完成（使用 {type(checkpointer).__name__}）"
        )
...
logger.info("[MediArch Supervisor] 图构建完成")
