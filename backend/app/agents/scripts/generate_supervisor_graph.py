"""
Utility for exporting the MediArch supervisor LangGraph as a flowchart image.

The script loads the compiled supervisor graph and writes either the Graphviz
PNG (matches ``images/supervisor_graph.png``) or a Mermaid definition so that
...
from backend.app.agents import supervisor_graph
DEFAULT_OUTPUT = PROJECT_ROOT / "images" / "supervisor_graph.png"
SupportedFormat = Literal["png", "mermaid", "mermaid-png"]


def _get_compiled_supervisor(rebuild: bool):
    """Return the compiled LangGraph instance, optionally forcing a rebuild."""
    if rebuild:
        logger.info("Rebuilding supervisor graph before export.")
        return supervisor_graph.build_supervisor_graph()
    return supervisor_graph.graph
...
def generate_supervisor_flowchart(
    ...
    *,
    ...
    """
    Render the supervisor graph diagram.
    ...
    Args:
        ...
        rebuild: Force a fresh compilation of the supervisor graph before drawing.

    Returns:
        Path to the generated artifact.
    """
    compiled_graph = _get_compiled_supervisor(rebuild=rebuild)
    ...
    logger.info("Supervisor graph exported to %s [%s]", output, fmt)
    return output


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export the MediArch LangGraph supervisor workflow diagram."
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--rebuild",
        action="store_true",
        help="Force recompiling the supervisor graph before rendering.",
    )
    parser.add_argument(
        "--allow-overwrite",
        action="store_true",
