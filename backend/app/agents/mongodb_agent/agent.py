...
async def node_rewrite_query(state: MongoDBState) -> Dict[str, Any]:
    ...
    # ✅ [2025-11-25] 提取 unified_hints（来自 Knowledge Fusion）
    unified_hints = {}
    neo4j_expansion = {}
    ...
    # ✅ [2025-11-25] 优先使用 unified_hints 的实体名作为额外查询词
    if unified_hints and unified_hints.get("entity_names"):
        hint_entity_names = unified_hints["entity_names"][:15]  # 最多15个
    ...
    # ✅ [2025-11-25] 使用 unified_hints 的搜索词
    if unified_hints and unified_hints.get("search_terms"):
        hint_search_terms = unified_hints["search_terms"][:10]
    ...
    logger.info(f"[MongoDB→Rewrite] 模式={mode}, search_terms={search_terms[:10]}...")
...
async def node_search_mongodb(state: MongoDBState) -> Dict[str, Any]:
    ...
    logger.info(f"[MongoDB->Search] 开始搜索，search_terms={search_terms[:10]}... (共{len(search_terms)}个)")
    ...
    explicit_page_window = max(0, min(explicit_page_window, 10))

    # ✅ [2025-11-25] 优先从 unified_hints 获取 chunk_ids
    unified_hints = {}
    if request and request.metadata:
        unified_hints = request.metadata.get("unified_hints", {})
    ...
    try:
        if chunk_ids:
            ...
            chunks = await asyncio.to_thread(retriever.get_chunks_by_ids, ids)
            used_strategy = "chunk_ids"

        else:
            # 模式2：关键词搜索（内置文本索引 + 回退策略）
            logger.info(f"[MongoDB→Search] 使用关键词搜索模式")
            chunks, used_strategy, retriever_diag = await asyncio.to_thread(
            ...
            if explicit_page_numbers and explicit_page_window == 0:
                ...
                for doc_id, pages in hints:
                    if len(extra_images) >= img_k:
                        break
                    per_doc = max(1, min(2, img_k - len(extra_images)))
                    candidates = await asyncio.to_thread(
                        retriever.get_image_chunks_near_pages,
                        doc_id,
                        pages,
                        per_doc,
                        explicit_page_window if explicit_page_numbers else 1,
                    )
        ...
        return {
            "retrieval_results": [],
            "diagnostics": {"error": f"search_failed: {e}"},
        }
    ...
    return {
        ...
        "diagnostics": {
            ...
            "search_terms": search_terms[:10],  # 只保存前10个
...
def _parse_section_hierarchy(section: str) -> tuple[str, str, str]:
    ...
    # 模式1: "第X章 标题 - X.Y 小节"
    match = re.match(r"(第\d+章)\s*([^\-]+?)(?:\s*-\s*(\d+\.\d+\s*.+))?$", section)
    if match:
    ...
    # 模式3: "第X章 标题"（只有章）
    match = re.match(r"(第\d+章)\s+(.+)", section)
    if match:
...
async def node_format_results(state: MongoDBState) -> Dict[str, Any]:
    ...
    for chunk in retrieval_results:
        ...
        if file_path:
            ...
            relative_path = re.sub(r'^.*?[/\\]backend[/\\]databases[/\\]documents[/\\]', '', file_path)
