# backend/tests/test_mediarch_graph.py
...
@pytest.mark.asyncio
async def test_mediarch_graph_basic_query():
    """测试基本查询流程（不使用真实 Workers）"""
    
    # Mock Worker Graphs
    with patch('backend.app.agents.mediarch_graph._get_worker_workflows') as mock_get_workers:
        ...
        # Mock Orchestrator
        with patch('backend.app.agents.mediarch_graph.orchestrator_logic_graph') as mock_orch:
            ...
            # Mock Synthesizer
            with patch('backend.app.agents.mediarch_graph.synth_graph') as mock_synth:
...
def test_mediarch_graph_items_dedup():
    """测试 items 去重功能"""
...
async def test_classify_feedback():
    ...
    # Mock LLM
    with patch('backend.app.agents.mediarch_graph.get_llm_manager') as mock_manager:
        mock_llm = AsyncMock()
        ...
        # 测试分类节点
        from backend.app.agents.mediarch_graph import node_classify_feedback
...
def test_route_after_feedback():
    """测试反馈后的路由决策"""
    from backend.app.agents.mediarch_graph import route_after_feedback
...
def test_prepare_retry_with_adjusted_params():
    """测试外部循环的参数调整"""
    from backend.app.agents.mediarch_graph import node_prepare_retry_with_adjusted_params
...
def test_prepare_new_question_with_history():
    """测试补充提问时保留对话历史"""
    from backend.app.agents.mediarch_graph import node_prepare_new_question_with_history
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("🧪 开始测试 MediArch Graph")
    print("=" * 60 + "\n")
    
    # 同步测试
    print("📋 测试 Reducer 功能...")
    test_mediarch_graph_items_dedup()
