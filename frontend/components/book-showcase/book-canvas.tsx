...
function LightingRig({
  ...
  useFrame(() => {
    if (spotlightRef.current) {
      spotlightRef.current.intensity = 1.6 + highlightIntensity * 1.4
      spotlightRef.current.angle = 0.45 + highlightIntensity * 0.12
    }
    if (fillLightRef.current) {
      fillLightRef.current.intensity = 0.85 + highlightIntensity * 0.75
    }
  })
  ...
  return (
    <>
      ...
      <hemisphereLight args={["#ffffff", "#dbeafe", 0.75]} />
      <directionalLight position={[8, 10, 6]} intensity={1.1} color="#ffffff" />
      <directionalLight position={[-8, 6, 6]} intensity={0.95} color="#eef6ff" />
      <spotLight
        ref={spotlightRef}
        position={[3, 7, 4]}
        angle={0.5}
        ...
        castShadow
      />
      <pointLight ref={fillLightRef} position={[-4, 2, 3]} intensity={0.85} color="#f8fbff" />
    </>
  )
}
